...
export interface TemplateContext {
   ...
   *
   * They become a dropdown on the Category column, for the same reason sizes
   * are one: a category typed by hand is a new category three screens later,
   * and the whole point of the controlled vocabulary is that Shopify's product
   * type, Loom's vocabulary and Sitoo's navigation all key off it. One category
   * is also pre-filled into every row, because a single-category batch should
   * not need the column touched at all.
   */
...
export async function buildImportTemplate(
  ...
  const categories = ctx.categories.filter((c) => c.name.trim());
  ...
  // --- Categories (hidden) — the Category column's source range -------------
  const catSheet = wb.addWorksheet(SHEET_CATEGORIES);
  catSheet.state = "veryHidden";
  catSheet.getColumn(1).width = 30;
  categories.forEach((c, i) => {
    catSheet.getCell(i + 1, 1).value = c.name;
  });
  ...
  for (let r = firstRow; r <= lastRow; r++) {
    ...
    ws.getCell(r, COL.category).dataValidation = {
      ...
      showErrorMessage: true,
      errorStyle: "stop",
      errorTitle: "Not a category in this batch",
      error:
        `This file was generated for: ${categories.map((c) => c.name).join(", ")}. ` +
        `Pick one of those — a category typed by hand is a new category to review, ` +
        `and it reaches Shopify, Loom and the till.`,
    };

    // One category means the column never has to be touched.
    if (categories.length === 1) ws.getCell(r, COL.category).value = categories[0].name;
  }
