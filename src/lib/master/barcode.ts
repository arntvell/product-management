...
/**
 * Reduce a barcode to the single form the master stores.
 *
 ...
 *    barcode; the EAN-13 is the UPC-A with a leading zero. We store 13.
 *
...
export function canonical(raw: string | null | undefined): string | null {
  ...
  if (stripped.length === 12) {
    // UPC-A -> EAN-13.
    const padded = "0" + stripped;
    return isValidEan13(padded) ? padded : null;
  }
  if (stripped.length === 13) {
    return isValidEan13(stripped) ? stripped : null;
  }
...
/** Why `canonical` rejected a value — for user-facing validation messages. */
...
/** Split a canonical barcode back into its range and sequence, if we own it. */
